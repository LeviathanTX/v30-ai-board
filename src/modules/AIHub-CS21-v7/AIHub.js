// V21 AI Hub v7 - Fixed upload functionality and enhanced features
...
export default function AIHub() {
  ...
  // Including Meeting Host in advisors - now with customAdvisors
  ...
  const advisors = [...defaultAdvisors, ...(state.customAdvisors || [])];
  ...
  // Fixed file upload handler that actually processes files
  const handleFileSelect = async (e) => {
    ...
    for (const file of files) {
      try {
        ...
        // Simulate processing (in production, this would call document processor)
        ...
        // Auto-share with advisors for discussion
        dispatch({ type: actions.SELECT_DOCUMENT, payload: processedDoc });
        
        dispatch({
          type: actions.ADD_NOTIFICATION,
          payload: { 
            message: `${file.name} uploaded and ready for AI analysis!`,
            type: 'success'
          }
        });
        ...
        const uploadMessage = {
          id: Date.now(),
          role: 'system',
          content: `📎 Document uploaded: ${file.name}. The AI advisors now have access to this document for analysis.`,
...
The meeting summary and action items will be available in your meeting history. Have a productive day!`,
  ...
  return (
    <div className="flex h-full bg-gray-50">
              ...
              {advisors.map(advisor => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{advisor.avatar_emoji || advisor.avatar}</span>
                  ...
                  <div className={`max-w-2xl ${msg.role === 'user' ? 'order-2' : 'order-1'}`}>
                    {msg.role === 'assistant' && msg.advisor && (
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-lg">{msg.advisor.avatar_emoji || msg.advisor.avatar}</span>
      ...
      {/* Hidden File Input - Fixed with proper onChange handler */}
      <input
        ref={fileInputRef}
        type="file"
        multiple
