...
export default function AIHub() {
  ...
  const handleFileSelect = (e) => {
    ...
    // Navigate to Document Hub with files
    dispatch({ 
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      for (const advisor of respondingAdvisors) {
        // Create a greeting response if this is the first message
        let responseContent = '';
        
        if (state.conversationMessages.length === 0) {
          responseContent = `Hello! I'm ${advisor.name}, your ${advisor.role}. ${advisor.isHost ? "I'll be facilitating today's advisory session. " : ''}I'm here to help with ${advisor.expertise.join(', ').toLowerCase()}. How can I assist you today?`;
        } else {
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {state.conversationMessages.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">Start Your AI Board Meeting</h3>
              <p className="text-gray-600 max-w-md mx-auto">
                Ask a question or share a business challenge to get insights from your AI advisors.
              </p>
            </div>
          ) : (
