-- V21 AI Board of Advisors - Supabase Database Schema (Fixed Version)
...
DROP INDEX IF EXISTS idx_advisors_is_custom;
...
DROP POLICY IF EXISTS "Everyone can view default advisors" ON advisors;
DROP POLICY IF EXISTS "Users can view own custom advisors" ON advisors;
DROP POLICY IF EXISTS "Users can create own advisors" ON advisors;
DROP POLICY IF EXISTS "Users can update own advisors" ON advisors;
DROP POLICY IF EXISTS "Users can delete own advisors" ON advisors;
...
-- Create or replace functions for common operations
...
-- The error about existing indexes has been handled