// src/components/CreateAdvisorModal/EnhancedCreateAdvisorModal.js
import React, { useState, useRef } from 'react';
import { X, User, Briefcase, Brain, Settings, Upload, FileText, Trash2, CheckCircle, Image, Camera, Mic, MicOff, Volume2 } from 'lucide-react';
...
export default function EnhancedCreateAdvisorModal({ isOpen, onClose, onCreateAdvisor }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await onCreateAdvisor(newAdvisor);

      // If documents were uploaded, create knowledge base
      if (uploadedDocuments.length > 0) {
        try {
          const { data: knowledgeBase, error: kbError } = await knowledgeBaseService.createKnowledgeBase(
            newAdvisor.id,
            `${newAdvisor.name} Knowledge Base`,
            `Uploaded documents for ${newAdvisor.name}`
          );
  ...
  const handleVoiceCommand = (command) => {
    setVoiceCommands(prev => `${prev}\nCommand: ${command.action} - ${command.value || command.originalText}`);
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">
                  Upload PDF, TXT, MD, or Word documents to create a knowledge base for your advisor
                </p>
                <p className="text-xs text-gray-500 mt-1">
